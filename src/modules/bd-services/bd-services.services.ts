...
export class BdServicesService {
  ...
  async atualizandoCliente(cliente: clientesTipo) {
    console.log('Atualizando/Criando cliente:', cliente.nome);
    const conexao = await connectToDatabase();
    const modelClientes = conexao.model(
      'clientesEco',
      clientesSchema,
    ...
    const clienteExistente = await modelClientes.findOne({
      nome: cliente.nome,
    });
    ...
    if (clienteExistente) {
      ...
      if (resultado.modifiedCount === 0) {
        console.log('Nenhuma modificação foi necessária.');
      } else {
    ...
    const todosClientes = await modelClientes.find({});
    console.log('Retornando lista atualizada de clientes.');
    return todosClientes;
  }
